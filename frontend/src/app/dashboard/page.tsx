...
function DashboardContent() {
  ...
  const recentEntries = useMemo(() => {
    ...
    const routineMap = new Map(
      routines.data?.data?.map((r: MorningRoutine) => [r.date, r]) || [],
    );
  ...
  const hasData =
    routines.data?.data?.length || productivity.data?.data?.length;
    ...
    <div className="space-y-8">
      ...
      {!isLoading && !hasData && !hasError && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div className="text-sm text-blue-800">
            ...
            <p>
              Start by importing a CSV file or adding entries manually. Showing
              demo data below.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatsCard
          title="Avg. Sleep"
          value={
            summary.data?.avg_sleep
              ? `${summary.data.avg_sleep.toFixed(1)} hrs`
              : "7.3 hrs"
          }
          subtitle="per night"
          trend="stable"
        ...
        <StatsCard
          title="Total Entries"
          value={String(
            summary.data?.total_entries ?? routines.data?.total ?? 0,
          )}
                ...
                <tbody>
                  {recentEntries.map((row) => (
                    <tr key={row.date} className="border-b border-slate-100">
                      ...
                      <td className="py-3 px-4 text-slate-700">{row.wake}</td>
                      <td className="py-3 px-4 text-slate-700">{row.sleep}</td>
