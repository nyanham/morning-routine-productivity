...
| **CORS**             | Cross-Origin Resource Sharing  — HTTP headers that control which browser origins may call the API.                       |
...
| **JWT**              | JSON Web Token  — a compact, URL-safe token format used for authentication. Supabase issues JWTs on login.               |
| **Mangum**           | An adapter that translates AWS API Gateway events into ASGI requests so FastAPI can run on Lambda.                      |
| **Middleware**       | Code that wraps every request/response cycle  — used here for CORS headers and exception handling.                       |
...
| **RLS**              | Row-Level Security  — PostgreSQL policies that restrict which rows a given user can read or write.                       |
...
| **SAM**              | AWS Serverless Application Model  — an IaC framework that extends CloudFormation for Lambda, API Gateway, etc.           |
