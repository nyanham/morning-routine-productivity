    ...
    for idx, row in df.iterrows():
        try:
            ...
            if routine_columns.issubset(csv_columns):
                ...
                routine_data = {k: v for k, v in row_dict.items() 
                               if k in ["date", "user_id", "wake_time", "sleep_duration_hours",
                                       "exercise_minutes", "meditation_minutes", "breakfast_quality",
                                       "morning_mood", "screen_time_before_bed", "caffeine_intake",
                                       "water_intake_ml"]}
                
                productivity_data = {k: v for k, v in row_dict.items()
                                    if k in ["date", "user_id", "productivity_score", "tasks_completed",
                                            "tasks_planned", "focus_hours", "distractions_count",
                                            "energy_level", "stress_level", "notes"]}
