...
class AnalyticsService:
    ...
    def get_summary(
        ...
        if productivity:
            avg_productivity = sum(p["productivity_score"] for p in productivity) / len(productivity)
        ...
        if len(productivity) >= 4:
            ...
            second_half_avg = sum(p["productivity_score"] for p in productivity[mid:]) / (len(productivity) - mid)
    ...
    def get_chart_data(
        ...
        routines_response = (
            self.supabase.table("morning_routines")
            .select("date, sleep_duration_hours, exercise_minutes, meditation_minutes, morning_mood")
            .eq("user_id", self.user_id)
            .gte("date", start_date.isoformat())
            .lte("date", end_date.isoformat())
            .order("date")
