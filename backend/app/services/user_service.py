...
class UserService:
    ...
    def update_profile(self, data: UserProfileUpdate) -> Optional[dict]:
    ...
    def update_settings(self, data: UserSettingsUpdate) -> Optional[dict]:
    ...
    def create_goal(self, data: UserGoalCreate) -> dict:
        ...
        if data.is_active:
            self.supabase.table("user_goals").update(
                {"is_active": False}
            ).eq("user_id", self.user_id).eq("goal_type", data.goal_type).eq("is_active", True).execute()
    ...
    def update_goal(self, goal_id: str, data: UserGoalUpdate) -> Optional[dict]:
        ...
        if payload.get("is_active"):
            ...
            if existing:
                self.supabase.table("user_goals").update(
                    {"is_active": False}
                ).eq("user_id", self.user_id).eq("goal_type", existing["goal_type"]).eq("is_active", True).neq("id", goal_id).execute()
