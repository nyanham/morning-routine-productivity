...
def generate_dataset(
    ...
    for day_offset in range(num_days):
        ...
        sleep_duration = round(
            np.clip(
                np.random.normal(base_sleep, 1.0) + (prev_sleep - 7) * 0.2,
                4.0, 10.0
            ), 1
        )
        ...
        mood_base = 5 + sleep_quality * 3 + (exercise_minutes > 0) * 1.5 + (meditation_minutes > 0) * 0.5
        ...
        stress_base = 6 - meditation_minutes * 0.1 - (exercise_minutes > 0) * 1.5 + (1 - sleep_quality) * 2
        ...
        focus_base = 3 + energy_level * 0.3 + (meditation_minutes > 0) * 0.5 + caffeine_optimal * 0.5
        focus_hours = round(np.clip(np.random.normal(focus_base, 1), 0, 8), 1)
        
        # Distractions (more with high caffeine, high stress, poor sleep)
        distraction_base = 3 + (caffeine_intake > 200) * 2 + stress_level * 0.3 - meditation_minutes * 0.1
        ...
        tasks_completed = int(np.clip(
            round(tasks_planned * completion_rate + np.random.normal(0, 1)),
            0, tasks_planned + 1  # Sometimes exceed planned!
        ))
...
def print_statistics(routines_df: pd.DataFrame, productivity_df: pd.DataFrame):
    ...
    print(f"   Exercise: {routines_df['exercise_minutes'].mean():.0f} min ({(routines_df['exercise_minutes'] > 0).mean()*100:.0f}% of days)")
    print(f"   Meditation: {routines_df['meditation_minutes'].mean():.0f} min ({(routines_df['meditation_minutes'] > 0).mean()*100:.0f}% of days)")
    ...
    print(f"   Tasks completed: {productivity_df['tasks_completed'].mean():.1f}/{productivity_df['tasks_planned'].mean():.1f}")
    print(f"   Distractions: {productivity_df['distractions_count'].mean():.1f}")
    
    # Correlations
    merged = routines_df.merge(productivity_df, on='date')
    print("\n🔗 KEY CORRELATIONS:")
    print(f"   Sleep → Productivity: {merged['sleep_duration_hours'].corr(merged['productivity_score']):.2f}")
    ...
    print(f"   Meditation → Stress: {merged['meditation_minutes'].corr(merged['stress_level']):.2f}")
    print(f"   Mood → Productivity: {merged['morning_mood'].corr(merged['productivity_score']):.2f}")
